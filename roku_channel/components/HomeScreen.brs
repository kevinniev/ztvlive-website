...
sub startHeroVideo()
    ...
    print "HomeScreen: Fetching live stream URL from API"
end sub

sub onStreamUrlLoaded()
    if m.streamTask = invalid or m.streamTask.response = invalid then
        print "HomeScreen: Failed to get stream URL, using fallback"
        playFallbackStream()
        return
    end if
    ...
    ' Check if it's a YouTube embed URL and extract the video ID
    if videoUrl <> invalid and videoUrl.instr("youtube.com/embed/") >= 0
        ' Can't play YouTube in Roku native player, use HLS fallback
        print "HomeScreen: YouTube URL detected, using HLS fallback"
        playFallbackStream()
        return
    end if
    
    ' Use the API-provided stream URL if available
...
sub playStream(videoUrl as string)
    ...
    print "HomeScreen: Hero video started with URL: " + videoUrl
end sub
